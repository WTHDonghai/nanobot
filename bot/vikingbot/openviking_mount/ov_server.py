...
class VikingClient:
    def __init__(self, agent_id: Optional[str] = None, override_account_id: Optional[str] = None):
        config = load_config()
        ...
        if openviking_config.mode == "local":
            ...
            # Use override_account_id if provided (for per-request multi-tenant routing)
            resolved_account_id = override_account_id or openviking_config.account_id
    ...
    async def create(cls, agent_id: Optional[str] = None, account_id: Optional[str] = None):
        ...
        Args:
            ...
            account_id: Optional account/tenant ID override. When provided, overrides
                        the config-level account_id for per-request multi-tenant routing.
        """
        instance = cls(agent_id=agent_id, override_account_id=account_id)
